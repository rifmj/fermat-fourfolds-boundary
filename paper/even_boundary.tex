...
implemented closure family and the cited results of the two-paper package --- is eight certified gap classes
($\nu\notin S_m$,
...
chain. Both proofs are internal to the two-paper package; Aoki's degree-form theorem and the two errata we record of its
...
Let $\alpha$ be a Hodge character of $X^4_m$ and let $\beta$ be either a Hodge character of
$X^4_m$ or the empty multiset $\varnothing$, for which $\nu(\varnothing):=0$ and
$\mathrm{claim}(\varnothing)$ is vacuously true. If $\nu(\alpha)-\nu(\beta)\in S_m$ and
$\mathrm{claim}(\beta)$ holds, then $\mathrm{claim}(\alpha)$ holds. (For
$\beta=\varnothing$ this is Aoki's \cite[Lemma~4.1(iii)]{Aoki00},
$\nu(\alpha)\in S_m\Rightarrow\mathrm{claim}(\alpha)$ --- the case used in
...
$(1,1)$), a standard block $\sigma_{p,x}$ with $p\in\{2,3,5\}$, $p\mid m$, and nonzero entries
(\cite[Thm.~2-1]{Aoki87} for $p=3,5$, with the inflation lemma when $\gcd(x,m/p)>1$;
Lemma~\ref{lem:p2neg}(i) for $p=2$) --- these have four entries for $p=2,3$ and six for $p=5$,
so the admitted block lengths are exactly $2,4,6$ --- or a grade-$3$ sextuple already
...
admits as blocks only pairs, grade-$2$ Hodge quadruples, the standard blocks with $p\le5$ above,
and grade-$3$ sextuples certified by the four base predicates --- block lengths exactly $2,4,6$.
Standard blocks with $p\ge7$ (length $p+1\ge8$) enter only the \emph{lattice} certificates of
\S\ref{sec:closures}, never the partition oracle; the restriction changes nothing in the tier reported
here: an exhaustive sweep over all forty classes and every prime $p\ge7$ dividing their level
finds no closure by such a block at depth $\le2$, nor at depth $\le3$ for the residual ten
(\texttt{long\_standard\_sweep.py} and its receipt in the ancillary). The deeper negative
campaigns behind the walls run to depth $4$--$5$ and were not re-run with long standard blocks;
the statement is therefore about the headline tier and the depth-$\le3$ residual check, not about
those sweeps. The \emph{deep tiers} add depth $k\le5$ and \emph{iteration}: a
...
survivor list, ship as \texttt{even\_tier\_table.json}. The standard runner recomputes the tier
predicates \emph{from the definitions} for every even $m\le60$ and checks the stored lists,
counts and canonical hashes at every even $m\le250$; a complete from-scratch regeneration of all
$123$ levels is a separate multi-hour command (\texttt{--emit}) and is not part of the smoke
runner. Every deep closure is stored as an
...
$(1,25,79,121,127,151)$, has indicated eigenvalue field $\Q(\sqrt{-3})$, and, numerically, $p=673$ appears as a kernel
prime of its Frobenius character, so the field-of-definition obstruction of \cite[Thm.~C]{BN}
does not fire at that prime (a computational observation, not part of any statement here).
...
The two $m=110$ classes are claim-equivalent --- the class with numerically indicated field $\Q(\zt_5)$ stands or falls with
the one with numerically indicated field $\Q(\zt_{110})$ --- and the three $m=114$ classes form a complete triangle at
...
\begin{proof}
The argument is internal to the two-paper package --- it uses the $*$-split transport theorem of
\cite{BN} and nothing from Aoki's degree-form theorem: an explicit $*$-split witness at level
$168$ plus the coset transfer of Proposition~\ref{prop:coset}. Put
\[
...
standard family present at this level is excluded by a residue count: the first seven entries of
$\sigma_{7,x}=(x,x+24,\dots,x+144,\,168-7x)$ all lie in the class $x\bmod24$, while the entries
of $a$ occupy the two classes $1,7\bmod24$ with multiplicities $3,3$, and each added vanishing
pair contributes at most one entry to a given class (the self-pair $(84,84)$ lies in the class
$12$), so at depth $\le2$ no class reaches multiplicity $7$. Within the implemented closure
family, and within the stated depth, generation and lift bounds, no route to $a$ itself was
found; what closes it is the transfer from the $*$-split class $\beta$ inside its own coset of
$S_{168}$.
\end{proof}
...
closure family and the cited results of the two-paper package --- is eight primitive classes in three
...
vectors). The coset congruence of Proposition~\ref{prop:w168} carries both kinds: a stored
integer combination over the generator list ($94$ coefficients in $[-3,3]$), re-summed from disk
by the standalone checker, and a modular kernel witness which --- because the difference lies in
$S_{168}$ --- separates $\nu(a)$ and $\nu(\beta)$ from the lattice simultaneously; the full census and exchange sweeps are reproducible from the supplied engines and are
...
\texttt{LICENSE} there). The same package is deposited as an immutable archival release under
the concept DOI \texttt{10.5281/zenodo.21670577}, which always resolves to the latest version;
that record carries both manuscripts of the two-paper package. The ancillary package is shared with the companion note: engines (census, closure
